...
import { runSensitivity, type SensitivityResult, type TornadoBar } from "@/lib/api";
  ...
  return (
    <div className="flex items-center gap-2 mb-2.5">
      ...
      <div className="w-20 text-right flex flex-col items-end">
        <span className="text-[10px] font-semibold text-[var(--text-secondary)]">{fmt(bar.spread)}</span>
      ...
      {result && (
        <>
          <p className="text-xs text-[var(--text-faint)] mb-3">
            Base {METRIC_OPTIONS.find((o) => o.value === result.target_metric)?.label || result.target_metric}: {fmt(result.base_metric_value)} | \u00B1{result.swing_pct}% swing
          </p>
              ...
              <ol className="list-decimal list-inside text-xs text-[var(--text-secondary)] space-y-0.5">
                {result.bars.map((b) => (
                  <li key={b.variable_id}>
                    <strong className="text-[var(--text-primary)]">{b.variable_name}</strong> &mdash; spread of {fmt(b.spread)} ({((b.spread / result.base_metric_value) * 100).toFixed(1)}% of base)
                  </li>
                ))}
              </ol>
            </div>
